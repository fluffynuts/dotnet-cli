import { ExecStepContext, Labelers } from "exec-step";
import {
    system,
    SystemError,
    SystemResult,
    sleep,
    quoteIfRequired as q,
    type Optional,
    type Dictionary
} from "system-wrapper";
...
import { fileExists, readTextFile, ls, FsEntities } from "yafs";
import { yellow } from "./ansi-colors";
import { pushIfSet, pushFlag } from "./cli-support";
...
import { parseNugetSources } from "./parse-nuget-sources";
import { updateNuspecVersion } from "./update-nuspec-version";
import { readNuspecVersion } from "./read-nuspec-version";
import { Version } from "./version";
import { cache } from "./cache";
import { resolveEnvFlag, resolveEnvNumber } from "./env";
import type {
    DotNetAddProjectToSolutionOptions,
    DotNetBaseOptions,
    DotNetBuildOptions,
    DotNetCleanOptions,
...
const debug = debugFactory("dotnet-cli");
...
function getByPath(obj: any, xmlPath: string): any {
...
function generateLabel(str: string): string {
...
export async function test(
    ...
    const labelText = !!opts.label
        ? `${generateLabel("Testing")}`
        : `${opts.label} ${generateLabel("Testing")}`;
    return runOnAllConfigurations(
        labelText,
        opts,
        configuration => {
...
export function incrementTempDbPortHintIfFound(env: Dictionary<string> | undefined): void {
    if (env === undefined) {
        return;
    }
...
export async function addNugetSource(
...
export async function removeNugetSource(
...
export async function enableNugetSource(
    source: string | NugetSource
): Promise<SystemResult> {
...
export async function disableNugetSource(
    source: string | NugetSource
): Promise<SystemResult> {
...
export async function tryFindConfiguredNugetSource(
...
export async function removeNugetSourceByName(
    find: string | Partial<NugetSource> | RegExp
): Promise<SystemResult> {
...
async function resolveAbsoluteNuspecPath(
    ...
    if (!resolvedPath) {
        throw new Error(`unable to resolve path to nuspec: no nuspec provided`);
    }
    ...
    async function resolveNuspecRelativeToProject(): Promise<string> {
        const
            containerDir = path.dirname(opts.target);
        const seek = path.resolve(path.join(containerDir, resolvedPath!));
...
async function shouldIncludeNuspec(
    ...
    if (!resolvedPath) {
        throw new Error(`Unable to resolve nuspec path`);
    }
...
export async function nugetPush(
...
function pushDisableBuildServers(args: string[], opts: DotNetCommonBuildOptions) {
...
async function determineDefaultNugetSource() {
    ...
    return defaultNugetSource = result;
}
...
async function runDotNetWith(
    ...
    let result: SystemResult | undefined;
    try {
...
function pushLoggers(args: string[], dotnetTestLoggers: Optional<DotNetTestLoggers>) {
    ...
    for (const loggerName of Object.keys(dotnetTestLoggers)) {
        ...
        args.push("--logger", `${toBuild.join(";")}`);
    }
}

function verifyIsSet<T>(obj: T, key: keyof T & string) {
    const value = obj[key];
    if (!value) {
...
// FIXME: where is this used outside of zarro?
export async function resolveContainerOptions(
    opts: DotNetPublishOptions
...
async function findFallbackContainerImageName(
    ...
    const assemblyName = await readAssemblyName(csproj);
    if (!assemblyName) {
        throw new Error(`unable to determine assembly name and no container image name set`);
    }
    return assemblyName;
}
...
export async function searchPackages(
    ...
    return await cache.through(
        ...
        resolveEnvNumber(cacheTTLEnvVar, defaultCacheTTL)
    );
}

export async function searchPackagesUncached(
    ...
    let rawResult: SystemResult | undefined;
    try {
...
export async function installPackage(
    opts: DotNetInstallNugetPackageOptions
): Promise<SystemResult> {
    if (!opts) {
...
export async function addProjectToSolution(
    ...
    if (!await fileExists(opts.solutionFile)) {
        throw new Error(`file not found: ${opts.solutionFile}`);
    }
    if (!await fileExists(opts.projectFile)) {
        throw new Error(`file not found: ${opts.projectFile}`);
    }
...
export async function listProjects(
    ...
    for (const line of rawResult.stdout) {
        const
            trimmed = line.trim(),
            seek = path.join(basePath, trimmed);
        if (await fileExists(seek)) {
            result.push(seek);
        }
    }
...
async function resolveSourceUrlFor(
    ...
    for (const src of sources) {
        if (src.name.toLowerCase() === lowered) {
            return src.url;
        }
    }
...
export async function upgradePackages(
    ...
    for (const project of projects) {
        ...
        for (const pkg of opts.packages) {
            const testFn = isRegex(pkg)
                ? (s1: string) => pkg.test(s1)
                : (s2: string) => s2.toLowerCase() === pkg.toLowerCase();

            for (const projectPackage of projectPackages) {
                if (testFn(projectPackage.id)) {
